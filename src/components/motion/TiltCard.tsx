...
export function TiltCard({ children, maxTilt = 10 }: Props) {
  ...
  const tiltX = useTransform(rx, (value) => `${value}deg`);
  const tiltY = useTransform(ry, (value) => `${value}deg`);
  ...
  return (
    <motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} style={{ rotateX: tiltX, rotateY: tiltY, transformStyle: "preserve-3d" }}>
      {children}
    </motion.div>
  );
}
